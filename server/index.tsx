...
app.use(
  cookieParser(process.env.COOKIE_SECRET || "omnomnomnom", {
    maxAge: 60 * 60 * 24 * 365,
  })
);
...
function page(component, payload?) {
  ...
  if (payload?.invite) {
    prepend = `<script type="application/json" id="invite">${JSON.stringify(
      payload?.invite
    )}</script>`;
  }
...
app.post("/api/presign", async (req, res) => {
  let { uploadUrl, viewUrl } = await presign(
    req.body.filename,
    req.body.filetype
  );
